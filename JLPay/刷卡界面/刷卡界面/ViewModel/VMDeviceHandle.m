...
//    if (self.deviceState == VMDeviceStateConnecting) { // 正在连接设备的要等到它完成连接
//        @weakify(self);
//        [[[RACObserve(self, deviceState) filter:^BOOL(NSNumber* state) { // 监控状态，直到状态为‘已连接’才进行断开
//            if (state.integerValue == VMDeviceStateConnected) {
//                return YES;
//            } else {
//                return NO;
//            }
