...
- (void) loadLastRegisterInfo:(NSDictionary*)lastRegisterInfo {
    for (NSString* key in lastRegisterInfo.allKeys) {
        if ([key isEqualToString:@"areaNo"]) { // 地区代码
            NSMutableDictionary* areaInfo = [self.arrayBasicInfo objectAtIndex:self.arrayBasicInfo.count - 1 - 1];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    NSInteger number = 0;
    if (section == 0) number = self.arrayBasicInfo.count - 1 /* 去掉email */;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([reuseIdentifier isEqualToString:IdentifierCellField]) {
        TextFieldCell* textCell = (TextFieldCell*)cell;
        textCell.curIndexPath = indexPath;
    }
    return cell;
}
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 2) {
        ...
        if (indexPath.row == self.arrayBasicInfo.count - 1 - 1) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([reuseIdentifier isEqualToString:IdentifierCellLabel]) {
        if (indexPath.section == 0 && indexPath.row == self.arrayBasicInfo.count - 1 - 1) {
...
- (void)tableViewCell:(id)cell didInputedText:(NSString *)text {
    ...
    if (tableCell.curIndexPath.section == 0) {
        NSMutableDictionary* basicInfo = [self.arrayBasicInfo objectAtIndex:tableCell.curIndexPath.row];
        [basicInfo setValue:text forKey:KeyInfoStringInputText];
        [basicInfo setValue:@(YES) forKey:KeyInfoBoolInputed];
    }
    else if (tableCell.curIndexPath.section == 1) {
        NSMutableDictionary* accountInfo = [self.arrayAccountInfo objectAtIndex:tableCell.curIndexPath.row];
...
- (void) packingBasicInfo {
    for (int i = 0; i < self.arrayBasicInfo.count; i++) {
        ...
        if (i != self.arrayBasicInfo.count - 1 - 1) { // 基本信息
                     ...
                     onIndexPath:(NSIndexPath*)indexPath
{
    ...
    if ([identifier isEqualToString:IdentifierCellField])
    {
        ...
        if (self.registerType == RegisterTypeRefused && [[self titleAtIndexPath:indexPath] isEqualToString:@"登陆用户名"]) {
            ...
            fieldCell.inputField.enabled = YES;
        }
    }
...
{
    // 设置数据源
    NSMutableDictionary* addrInfo = [self.arrayBasicInfo objectAtIndex:self.arrayBasicInfo.count - 1 - 1];
    ...
    NSIndexPath* indexPath = [NSIndexPath indexPathForRow:self.arrayBasicInfo.count - 1 - 1 inSection:0];
...
- (NSArray *)arrayBasicInfo {
    if (_arrayBasicInfo == nil) {
        ...
        NSArray* basicKeys = @[KeyInfoStringTitle,
                               KeyInfoStringPlayceHolder,
                               KeyInfoBoolMustInput,
                               KeyInfoStringInputText,
                               KeyInfoBoolSecureEnable,
                               KeyInfoBoolInputed,
                               KeyInfoStringKeyName,
                               KeyInfoIntLengthLimit];
        NSArray* areaKeys = @[KeyInfoStringTitle,
                              KeyInfoBoolMustInput,
                              KeyInfoStringPlayceHolder,
                              KeyInfoStringAreaCode,
                              KeyInfoStringDetailArea,
                              KeyInfoBoolInputed,
                              KeyInfoStringDetailKeyName,
                              KeyInfoStringAreaCodeKeyName];
