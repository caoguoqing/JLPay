...
- (BOOL)connectToHost:(NSString *)hostname
          ...
          withTimeout:(NSTimeInterval)timeout
                error:(NSError **)errPtr
              receive:(NSString *)theip{
    receiveTheIpStr=theip;
    if (theDelegate == NULL)
    {
        [NSException raise:AsyncSocketException
                    format:@"Attempting to connect without a delegate. Set a delegate first."];
...
- (void)completeCurrentRead
{
     ...
     */
    if ([receiveTheIpStr isEqualToString:@""]) {
        if([theDelegate respondsToSelector:@selector(onSocket:didReadData:withTag:)])
        {
            [theDelegate onSocket:self didReadData:result withTag:theCurrentRead->tag];
        }
    }else{
        if([theDelegate respondsToSelector:@selector(onSocket:didReadData:withTag: receiveIp:)])
        {
            [theDelegate onSocket:self didReadData:result withTag:theCurrentRead->tag receiveIp:receiveTheIpStr];
        }
    }
