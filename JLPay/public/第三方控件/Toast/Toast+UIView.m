...
static const CGFloat CSToastDefaultDuration     = 1.5; // not enough review time for 1.0s
...
- (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    if([point isKindOfClass:[NSString class]]) {
        ...
        if([point caseInsensitiveCompare:@"top"] == NSOrderedSame) {
            ...
            return CGPointMake(self.bounds.size.width/2, (self.bounds.size.height - (toast.frame.size.height / 2)) - CSToastVerticalPadding - 45);  // 将提示框上移 45 像素;
        } else if([point caseInsensitiveCompare:@"center"] == NSOrderedSame) {
            return CGPointMake(self.bounds.size.width / 2, self.bounds.size.height / 2-20);
        }
